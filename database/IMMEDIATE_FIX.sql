-- IMMEDIATE FIX FOR UUID ERROR
-- This script will fix the UUID issue right now

-- Step 1: Drop and recreate users table with TEXT id
...
-- Step 2: Drop and recreate bets table with TEXT user_id
...
-- Step 3: Drop and recreate wrestlecoin_transactions table with TEXT user_id
...
CREATE POLICY "Users can read own bets" ON bets FOR SELECT USING (auth.uid()::text = user_id::text);
CREATE POLICY "Users can insert own bets" ON bets FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);
CREATE POLICY "Users can read own transactions" ON wrestlecoin_transactions FOR SELECT USING (auth.uid()::text = user_id::text);
CREATE POLICY "Users can insert own transactions" ON wrestlecoin_transactions FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);

-- Step 6: Create user creation function
...
-- Step 7: Create match pool update function
...
SELECT 'UUID error fixed! Database schema updated with TEXT user_id columns.' as message;
