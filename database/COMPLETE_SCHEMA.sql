-- COMPLETE WRESTLEBET DATABASE SCHEMA
-- Single consolidated schema for all betting functionality
...
-- Matches table with betting pools
...
-- Function to update match pools and odds
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    ...
    SET 
      wrestler1_pool = (
        ...
        FROM bets 
        WHERE match_id = NEW.match_id AND wrestler_choice = 'wrestler1' AND status = 'active'
      ),
      wrestler2_pool = (
        SELECT COALESCE(SUM(bet_amount), 0) 
        FROM bets 
        WHERE match_id = NEW.match_id AND wrestler_choice = 'wrestler2' AND status = 'active'
      ),
      total_pool = (
        SELECT COALESCE(SUM(bet_amount), 0) 
        FROM bets 
...
-- SUCCESS MESSAGE
-- ============================================================================
SELECT 'Complete WrestleBet database schema deployed successfully!' as message,
       'All tables, functions, triggers, and policies are ready for betting.' as status;
