...
-- Step 3: Create matches table (if it doesn't exist)
CREATE TABLE IF NOT EXISTS matches (
  ...
  wrestler1_odds DECIMAL(5,2) DEFAULT 1.0,
  wrestler2_odds DECIMAL(5,2) DEFAULT 1.0,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Step 4: Drop and recreate bets table with TEXT user_id
...
-- Step 5: Drop and recreate wrestlecoin_transactions table with TEXT user_id
...
-- Step 7: Create basic RLS policies (with unique names)
...
-- Step 8: Drop existing function first, then create user creation function
DROP FUNCTION IF EXISTS create_user_if_not_exists(TEXT);

CREATE FUNCTION create_user_if_not_exists(p_clerk_user_id TEXT)
RETURNS TABLE(user_id TEXT, wrestlecoin_balance INTEGER) AS $$
BEGIN
  ...
  RETURN QUERY
  SELECT u.id as user_id, u.wrestlecoin_balance
  FROM users u
  WHERE u.id = p_clerk_user_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Step 9: Drop existing function first, then create match pool update function
